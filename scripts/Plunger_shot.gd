...
func _ready():
	frame_pointer = 0 
	$Sprite.set_texture(plunger_fire_frames[0])
...
func move(delta_y = 4):	
	if status == true:
		...
		if position.y<231 and status == true:
			...
			emit_signal('plunger_shot_ended')
			explode(0.15)
...
func get_texture():
	frame_pointer+=1
	if frame_pointer == plunger_fire_frames.size():
		frame_pointer = 0
	return plunger_fire_frames[frame_pointer]
