...
onready var large_invader_scene = preload("res://scenes/Invader_large.tscn")
onready var medium_invader_scene = preload("res://scenes/Invader_medium.tscn")
onready var small_invader_scene = preload("res://scenes/Invader_small.tscn")

var spawn_rows = [ 120,144,160,168,168,168,176,176,176 ]
var spawn_rows_pointer = 0
...
func move_across():
	var invaders = get_invaders()  #get invaders in group
	var size = invaders.size() - 1
	
	if invader_pointer <= size:
		var invader = invaders[invader_pointer]
...
func move_down():
	if invader_pointer <= get_child_count():
...
func check_move_limits()-> bool:
	for invader in get_invaders():
		if invader.position.x + invader.width >= right_margin and invader_direction>0:
			return true
...
func spawn_invaders():
	...
	for y in range (invader_cols):
		for x in range (invader_rows):
			var invader
			match y:
    		0,1: invader = large_invader_scene.instance()
		    2,3: invader = medium_invader_scene.instance()
    		4: invader = small_invader_scene.instance()
...
func _on_invader_hit(area,score_add,invader_index):
	...
	emit_signal("hit")
	emit_signal("score_add", score_add) #change name to points_awarded or something better
